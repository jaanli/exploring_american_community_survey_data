    ...
    for header, details in data_dict.items():
        if "Values" in details:
                if header in df_headers.columns:
                    # enum_values = [f"'{clean_enum_value(value)}'" for value in details["Values"].values() if ("integer weight" not in value.lower() and "identifier" not in value.lower() and "codes" not in value.lower())]
                    enum_values = [f"'{key}'" for key, value in details["Values"].items() if ("integer weight" not in value.lower() and "identifier" not in value.lower() and "codes" not in value.lower() and "number" not in value.lower() and "age" not in value.lower())]
...
{newline_with_comma.join(column_definitions)}
);

INSERT INTO {materialized_name}
